...
The site launched with solid technical foundations (SEO, dark mode, accessibility, contact backend) but the visual experience is static — sections appear flat, page transitions are hard cuts, and there is no floating booking CTA. Modern performer portfolios (Wayne McGregor, Hofesh Shechter, Rachele Perla) all use scroll-driven reveals and cinematic page transitions as a baseline. Visitors judge a performer's quality within 3 seconds. A site that doesn't move feels like a brochure from 2019.
...
5. **Floating booking CTA** — a fixed-position "Book Nikoleta" button that slides in after scrolling past the hero (≈300px). Suppressed on `/contact` (same logic as the existing `BookingCTA`). Pure CSS + a `useScrollDepth` hook (no library).
...
9. **Video hero** — `heroVideoSrc` field already exists in `profile.ts` (commented out). One-line uncomment when Nikoleta provides an MP4. No build work needed.

10. **Calendly embed** — one embed snippet on the Contact page. Requires Nikoleta to create a Calendly account and configure availability.
...
- `floating-cta`: Floating booking CTA behaviour — trigger depth, suppression rules, z-index contract.
...
- No 3D, WebGL, or scroll-parallax depth effects — too heavy for a solo performer site, and content isn't ready for it.
