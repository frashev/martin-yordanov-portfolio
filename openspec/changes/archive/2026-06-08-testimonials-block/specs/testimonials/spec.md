...
- **WHEN** the user scrolls the testimonials section into view (reduced motion inactive)
- **THEN** each card fades/slides into view with a staggered delay

#### Scenario: Reduced motion respected

- **WHEN** `prefers-reduced-motion: reduce` is set in the OS
- **THEN** cards appear immediately without animation
