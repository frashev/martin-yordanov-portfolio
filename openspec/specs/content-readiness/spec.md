# Spec: Content Readiness

## Purpose

Defines how the site handles real images as they are progressively introduced by the client — lazy-loading with fade-in, graceful error states, a backward-compatible `PlaceholderImage` upgrade path, a hero photo slot on the Home page, and a cleaned-up Gallery rendering path.
...
#### Scenario: Image fades in on load

- **WHEN** a GalleryImage's image finishes loading
- **THEN** the image transitions from invisible to fully visible with a smooth fade

#### Scenario: Image also fades in under reduced-motion
