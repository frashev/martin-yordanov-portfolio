...
Defines the documentation artifacts that accompany the Supabase backend: the runnable SQL DDL for the contact messages table and the staged backend roadmap covering V1 through V4.

## Requirements

### Requirement: SQL DDL documentation for contact_messages

The repository SHALL contain `docs/supabase/contact-backend.sql` with runnable SQL that creates the `public.contact_messages` table, enables RLS, and applies the insert-only anon policy as specified in the `contact-backend` spec.
...
- **V2 — Future secure backend layer**: Cloudflare Workers Functions, server-side secrets, `/api/contact` endpoint, same contact service interface.
...
- **V4 — Future payments**: Cloudflare Workers Functions or Supabase Edge Functions for payment endpoints, Stripe Checkout, server-side secret handling, webhooks.
