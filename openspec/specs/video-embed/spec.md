...
Defines how video cards on the Videos page render a responsive iframe when an embed URL is available, and how they fall back to a polished placeholder state when no URL is set — making it safe to swap in real embed links later without touching the surrounding layout.
